...
public interface AddLeadRepository extends JpaRepository<AddLead, Long> {
    ...
    @Query("SELECT al.createdDate, COUNT(al) " +
            "FROM AddLead al " +
            "WHERE al.clients.id = :clientId " +
            "AND FUNCTION('YEAR', al.createdDate) = :year " +
            "AND FUNCTION('MONTH', al.createdDate) = :month " +
            "GROUP BY al.createdDate " +
            "ORDER BY al.createdDate")
     List<Object[]> countLeadsPerDay(
